...
import com.albertoventurini.schemino.truffle.exceptions.TypeMismatch;
...
public class TypedObject {
    ...
    public long getLongOrThrow() {
        if (type != ScheminoType.LONG) {
            throw new TypeMismatch(ScheminoType.LONG, type, null);
        }
        return (long) value;
    }
    ...
    public boolean getBooleanOrThrow() {
        if (type != ScheminoType.BOOLEAN) {
            throw new TypeMismatch(ScheminoType.BOOLEAN, type, null);
        }
...
//    }
    ...
    public ScheminoList getListOrThrow() {
        if (type != ScheminoType.LIST) {
            throw new TypeMismatch(ScheminoType.LIST, type, null);
        }
